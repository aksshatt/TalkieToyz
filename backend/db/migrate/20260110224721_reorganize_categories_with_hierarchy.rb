class ReorganizeCategoriesWithHierarchy < ActiveRecord::Migration[7.1]
  def up
    # Check if already reorganized (look for hierarchical domains)
    return if Category.exists?(slug: 'physical-domain')
  ...
  def down
    # Don't reverse this migration as it would delete all categories
