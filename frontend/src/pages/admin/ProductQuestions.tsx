  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-warmgray-800 dark:text-warmgray-200">Product Q&amp;A</h1>
        ...
        <form onSubmit={handleSearch} className="flex gap-2">
          <input
            ...
            className="px-3 py-2 border border-warmgray-300 dark:border-surface-dark-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-400"
          />
          <button
            type="submit"
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
              filter === t.key
                ...
                : 'bg-white dark:bg-surface-dark-raised text-warmgray-600 dark:text-warmgray-400 border border-warmgray-200 dark:border-surface-dark-border hover:bg-warmgray-50 dark:hover:bg-white/5 dark:bg-surface-dark'
            }`}
          >
            {t.label}
          </button>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {questions.map((q) => (
            <div key={q.id} className="bg-white dark:bg-surface-dark-raised rounded-xl border border-warmgray-200 dark:border-surface-dark-border p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-warmgray-800 dark:text-warmgray-200 font-medium mb-2">{q.question}</p>
                  ...
                  {q.answered && q.answer && (
                    <div className="bg-teal-50 rounded-lg p-3 mt-2">
                      ...
                      <p className="text-sm text-warmgray-700 dark:text-warmgray-300">{q.answer}</p>
                    </div>
                  )}
                  ...
                  {showAnswerForm === q.id && (
                    <div className="mt-3 flex gap-2">
                      <textarea
                        ...
                        className="flex-1 px-3 py-2 border border-warmgray-300 dark:border-surface-dark-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none"
                      />
                      <button
