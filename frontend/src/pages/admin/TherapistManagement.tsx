...
const TherapistManagement: React.FC = () => {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-warmgray-900 dark:text-warmgray-100">Therapist Management</h1>
      ...
      {therapistsLoading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-surface-dark-raised rounded-3xl border-2 border-dashed border-warmgray-200 dark:border-surface-dark-border">
          ...
          <p className="font-semibold text-warmgray-600 dark:text-warmgray-400">No approved therapists found</p>
        </div>
      ) : (
        <div className="space-y-3">
          {therapists.map(therapist => {
            ...
            return (
              <motion.div key={therapist.id} layout className="bg-white dark:bg-surface-dark-raised rounded-2xl shadow-soft border border-warmgray-100 overflow-hidden">
                {/* Therapist Row */}
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                      ...
                      {assignments.length === 0 ? (
                        ...
                        <div className="space-y-2 pt-3">
                          {assignments.map(a => (
                            <div key={a.id} className="flex items-center gap-3 bg-warmgray-50 dark:bg-surface-dark rounded-xl p-3">
        ...
        {assignModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              className="bg-white dark:bg-surface-dark-raised rounded-3xl shadow-soft-xl p-6 max-w-md w-full max-h-[80vh] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-warmgray-900 dark:text-warmgray-100">Assign Patient to {assignModal.therapistName}</h3>
              ...
              <div className="flex-1 overflow-y-auto space-y-2">
                {patientsLoading ? (
                  ...
                  <motion.button key={p.id} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                    ...
                    className="w-full flex items-center gap-3 p-3 bg-warmgray-50 dark:bg-surface-dark hover:bg-teal-light/20 rounded-xl transition-colors text-left disabled:opacity-60">
