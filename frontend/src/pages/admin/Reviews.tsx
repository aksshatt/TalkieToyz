  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-warmgray-100">Reviews Moderation</h1>
          <p className="text-gray-500 dark:text-warmgray-500 text-sm mt-1">Approve, reject, or respond to customer reviews</p>
        </div>
      </div>

      {/* Filters */}
      <div className="bg-white dark:bg-surface-dark-raised rounded-xl shadow-sm border border-gray-200 dark:border-surface-dark-border p-4 flex flex-wrap gap-3 items-center">
        <div className="flex rounded-lg overflow-hidden border border-gray-200 dark:border-surface-dark-border">
          {(['pending', 'approved', 'all'] as Filter[]).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2 text-sm font-medium capitalize ${filter === f ? 'bg-purple-600 text-white' : 'bg-white dark:bg-surface-dark-raised text-gray-600 dark:text-warmgray-400 hover:bg-gray-50 dark:hover:bg-white/5 dark:bg-surface-dark'}`}
            >
              {f}
            </button>
          ))}
        ...
        <form onSubmit={e => { e.preventDefault(); load(); }} className="flex gap-2 flex-1 max-w-sm">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border border-gray-200 dark:border-surface-dark-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
          </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => <div key={i} className="h-32 bg-white dark:bg-surface-dark-raised rounded-xl animate-pulse border border-gray-200 dark:border-surface-dark-border" />)}
          </div>
        ) : reviews.length === 0 ? (
          <div className="bg-white dark:bg-surface-dark-raised rounded-xl border border-gray-200 dark:border-surface-dark-border p-12 text-center text-gray-500 dark:text-warmgray-500">
            No reviews found
          </div>
        ) : reviews.map(review => (
          <div key={review.id} className="bg-white dark:bg-surface-dark-raised rounded-xl shadow-sm border border-gray-200 dark:border-surface-dark-border p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2 flex-wrap">
                  <span className="font-semibold text-gray-900 dark:text-warmgray-100">{review.user?.name || 'Anonymous'}</span>
                ...
                {review.product?.name && (
                  <p className="text-xs text-gray-500 dark:text-warmgray-500 mb-2">Product: <span className="font-medium">{review.product.name}</span></p>
                )}
                <p className="text-gray-700 dark:text-warmgray-300 text-sm">{review.comment}</p>
                ...
                {review.admin_response && (
                  <div className="mt-3 pl-3 border-l-2 border-purple-200">
                    ...
                    <p className="text-sm text-gray-600 dark:text-warmgray-400">{review.admin_response}</p>
                  </div>
                )}
                ...
                {showResponseForm === review.id && (
                  <div className="mt-3 flex gap-2">
                    <textarea
                      ...
                      rows={2}
                      className="flex-1 px-3 py-2 border border-gray-200 dark:border-surface-dark-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
                    />
                    <div className="flex flex-col gap-1">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-gray-100 dark:bg-surface-dark-border text-gray-600 dark:text-warmgray-400 rounded-lg text-sm hover:bg-gray-200"
                      >
                        Cancel
                      </button>
                    </div>
