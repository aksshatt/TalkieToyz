    ...
    <div className="min-h-screen bg-warmgray-50 dark:bg-surface-dark">
      <div className="max-w-3xl mx-auto p-4">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-[var(--font-family-fun)] font-bold text-warmgray-900 dark:text-warmgray-100">
        ...
        {!selectedConvId ? (
          ...
          convsLoading ? (
            ...
            <div className="bg-white dark:bg-surface-dark-raised rounded-3xl shadow-soft p-12 text-center">
              ...
              <p className="font-bold text-warmgray-800 dark:text-warmgray-200 text-lg mb-2">No messages yet</p>
            ...
            <div className="space-y-2">
              {conversations.map(conv => (
                <motion.button key={conv.id} whileHover={{ scale: 1.005 }} whileTap={{ scale: 0.995 }}
                  ...
                  className="w-full flex items-center gap-4 bg-white dark:bg-surface-dark-raised rounded-2xl p-4 shadow-soft hover:shadow-soft-md transition-all border border-warmgray-100 dark:border-surface-dark-border hover:border-teal/30 text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-warmgray-900 dark:text-warmgray-100">{conv.therapist?.name}</p>
          ...
          <div className="bg-white dark:bg-surface-dark-raised rounded-3xl shadow-soft border border-warmgray-100 dark:border-surface-dark-border flex flex-col" style={{ height: 'calc(100vh - 180px)' }}>
                    ...
                    <div className={`max-w-[75%] flex items-end gap-2 ${isPatient ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      <div>
                        {msg.message_type === 'text' ? (
                          <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isPatient ? 'bg-coral-light/20 text-warmgray-800 dark:text-warmgray-200 rounded-br-sm' : 'bg-teal-light/20 text-warmgray-800 dark:text-warmgray-200 rounded-bl-sm'}`}>
                            {msg.content}
                          </div>
                        ) : msg.message_type === 'product' ? (
                          <Link to={`/products/${msg.metadata?.product_slug}`}
                            className="block bg-warmgray-50 dark:bg-surface-dark hover:bg-teal-light/10 border-2 border-warmgray-200 dark:border-surface-dark-border hover:border-teal/30 rounded-2xl p-3 min-w-[200px] transition-colors">
                          ...
                          <Link to={`/assessments/${msg.metadata?.assessment_slug}`}
                            className="block bg-warmgray-50 dark:bg-surface-dark hover:bg-sky-light/10 border-2 border-warmgray-200 dark:border-surface-dark-border hover:border-sky/30 rounded-2xl p-3 min-w-[200px] transition-colors">
            ...
            {/* Input */}
            <form onSubmit={handleSend} className="p-4 border-t border-warmgray-100 dark:border-surface-dark-border flex gap-3">
              <input
                value={newMessage}
                onChange={e => setNewMessage(e.target.value)}
                placeholder="Type a message…"
                className="flex-1 px-4 py-2.5 border-2 border-warmgray-200 dark:border-surface-dark-border rounded-2xl text-sm focus:border-teal focus:outline-none"
