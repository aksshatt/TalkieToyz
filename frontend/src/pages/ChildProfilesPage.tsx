...
const ProfileForm: React.FC<{ initial?: Partial<ChildProfile>; onSave: (data: any) => void; onCancel: () => void; loading: boolean }> = ({
  ...
  return (
    <div className="bg-white dark:bg-surface-dark-raised rounded-2xl border border-gray-200 dark:border-surface-dark-border p-6 shadow-sm">
      <h3 className="font-bold text-lg text-gray-900 dark:text-warmgray-100 mb-5">{initial?.id ? 'Edit' : 'Add'} Child Profile</h3>
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-gray-600 dark:text-warmgray-400 mb-1">Child's Name *</label>
          <input
            className="w-full border border-gray-300 dark:border-surface-dark-border rounded-xl px-4 py-2.5 text-sm"
            value={form.name}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 dark:text-warmgray-400 mb-1">Date of Birth *</label>
          <input
            type="date"
            className="w-full border border-gray-300 dark:border-surface-dark-border rounded-xl px-4 py-2.5 text-sm"
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 dark:text-warmgray-400 mb-2">Speech Goals (select all that apply)</label>
              ...
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border transition ${
                  form.speech_goals.includes(g)
                    ...
                    : 'border-gray-300 dark:border-surface-dark-border text-gray-600 dark:text-warmgray-400 hover:border-indigo-400'
                }`}
              >
                {g}
              </button>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 dark:text-warmgray-400 mb-2">Profile Color</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 dark:text-warmgray-400 mb-1">Notes (optional)</label>
          <textarea
            className="w-full border border-gray-300 dark:border-surface-dark-border rounded-xl px-4 py-2.5 text-sm"
            rows={2}
            value={form.notes}
      ...
      <div className="flex gap-3 mt-5">
        ...
        <button onClick={onCancel} className="px-4 py-2.5 border border-gray-300 dark:border-surface-dark-border rounded-xl text-sm text-gray-600 dark:text-warmgray-400 hover:bg-gray-50 dark:hover:bg-white/5 dark:bg-surface-dark">
          Cancel
        </button>
      </div>
    </div>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-surface-dark py-8 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-white dark:bg-surface-dark-raised rounded-2xl border border-dashed border-gray-300 dark:border-surface-dark-border">
              <User className="w-12 h-12 text-gray-300 mx-auto mb-3" />
              <h3 className="font-semibold text-gray-600 dark:text-warmgray-400 mb-1">No child profiles yet</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profiles.map(profile => (
                editingProfile?.id === profile.id ? (
                  ...
                  <div key={profile.id} className="bg-white dark:bg-surface-dark-raised rounded-2xl border border-gray-200 dark:border-surface-dark-border p-5 hover:shadow-md transition">
                    ...
                    <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-surface-dark-border">
                      <Link
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 dark:text-warmgray-400"
                      >
