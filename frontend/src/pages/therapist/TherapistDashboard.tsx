...
const TherapistDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-[var(--font-family-fun)] font-bold text-warmgray-900">Dashboard</h1>
        <p className="text-warmgray-500 text-sm mt-1">Overview of your patients and activity</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl p-5 shadow-soft border border-warmgray-100">
          <Users className="h-8 w-8 text-teal mb-3" />
          <p className="text-3xl font-bold text-warmgray-900">{patients.length}</p>
          <p className="text-sm text-warmgray-500 mt-1">Assigned Patients</p>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-soft border border-warmgray-100">
          <MessageSquare className="h-8 w-8 text-sky mb-3" />
          <p className="text-3xl font-bold text-warmgray-900">{unread}</p>
          <p className="text-sm text-warmgray-500 mt-1">Unread Messages</p>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-soft border border-warmgray-100 col-span-2 sm:col-span-1">
          <Award className="h-8 w-8 text-coral mb-3" />
          <p className="text-3xl font-bold text-warmgray-900">{creds.experience_years ?? '—'}</p>
          <p className="text-sm text-warmgray-500 mt-1">Years Experience</p>
        </div>
      </div>

      {/* Recent Patients */}
      <div className="bg-white rounded-2xl shadow-soft border border-warmgray-100 p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-warmgray-900">Recent Patients</h2>
          ...
          <div className="space-y-3">
            {patients.slice(0, 5).map((p: any) => (
              <Link key={p.id} to={`/therapist/patients/${p.id}`}
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-warmgray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-warmgray-900 text-sm">{p.name}</p>
      ...
      <div className="bg-white rounded-2xl shadow-soft border border-warmgray-100 p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-warmgray-900 flex items-center gap-2">
        ...
        {!editingCreds ? (
          ...
          <div className="space-y-4 text-sm">
            <div>
              <label className="block text-warmgray-600 mb-1">Years of Experience</label>
              <input type="number" min={0} value={credForm.experience_years || ''} onChange={e => setCredForm((f: any) => ({ ...f, experience_years: e.target.value }))}
                className="w-32 px-3 py-2 border border-warmgray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal" />
            </div>
            <div>
              <label className="block text-warmgray-600 mb-1">Specializations (comma-separated)</label>
              <input type="text" value={(credForm.specializations || []).join(', ')}
                ...
                className="w-full px-3 py-2 border border-warmgray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal" placeholder="e.g. Speech Delay, Autism, Aphasia" />
            </div>
            <div>
              <label className="block text-warmgray-600 mb-1">Languages Spoken (comma-separated)</label>
              <input type="text" value={(credForm.languages_spoken || []).join(', ')}
                ...
                className="w-full px-3 py-2 border border-warmgray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal" placeholder="e.g. Hindi, English, Tamil" />
            </div>
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-warmgray-600">Certifications</label>
                ...
                {(credForm.certifications || []).map((cert: any, idx: number) => (
                  <div key={idx} className="flex gap-2 items-start">
                    <input placeholder="Name" value={cert.name} onChange={e => updateCert(idx, 'name', e.target.value)}
                      className="flex-1 px-2 py-1.5 border border-warmgray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-teal text-xs" />
                    ...
                    <input placeholder="Year" type="number" value={cert.year} onChange={e => updateCert(idx, 'year', Number(e.target.value))}
                      className="w-20 px-2 py-1.5 border border-warmgray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-teal text-xs" />
