...
export default function NotificationPreferences() {
  ...
  if (isLoading || !prefs) return (
    <div className="max-w-xl mx-auto px-4 py-12 space-y-3">
      {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-16 bg-white rounded-2xl animate-pulse border border-warmgray-100" />)}
    </div>
  );
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-warmgray-900">Notification Preferences</h1>
        <p className="text-sm text-warmgray-500 mt-1">Choose how and what you want to hear from us</p>
      </div>

      <div className="bg-white rounded-2xl border border-warmgray-100 shadow-soft divide-y divide-warmgray-50">
        {items.map(({ key, label, desc, Icon }) => (
          <div key={key} className="flex items-center gap-4 p-4">
            <div className="w-9 h-9 rounded-xl bg-warmgray-50 flex items-center justify-center flex-shrink-0">
              <Icon className="h-4.5 w-4.5 text-warmgray-500" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-medium text-warmgray-900 text-sm">{label}</p>
            ...
            >
              <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${prefs[key as keyof Prefs] ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
        ))}
      </div>
