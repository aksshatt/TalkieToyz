...
const BookAppointmentModal = ({ isOpen, onClose }: BookAppointmentModalProps) => {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-surface-dark-raised rounded-2xl shadow-soft-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-warmgray-200 dark:border-surface-dark-border">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-teal-gradient rounded-full">
              <Calendar className="h-5 w-5 text-white" />
            </div>
            <h2 className="font-bold text-lg text-warmgray-900 dark:text-warmgray-100">Book an Appointment</h2>
          </div>
          <button onClick={handleClose} className="p-1 text-warmgray-500 hover:text-warmgray-700 dark:text-warmgray-300">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {submitted ? (
          <div className="p-8 text-center">
            ...
            <h3 className="font-bold text-xl text-warmgray-900 dark:text-warmgray-100 mb-2">Request Submitted!</h3>
            <p className="text-warmgray-600 dark:text-warmgray-400 mb-2">We'll contact you shortly to confirm your appointment.</p>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            <p className="text-sm text-warmgray-600 dark:text-warmgray-400 bg-teal-light/20 p-3 rounded-lg">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border-2 border-warmgray-300 dark:border-surface-dark-border rounded-lg text-sm focus:border-teal focus:outline-none"
            ...
            <div>
              ...
              {form.preferred_language === 'Others' && (
                <input
                  ...
                  className="mt-2 w-full px-3 py-2 border-2 border-warmgray-300 dark:border-surface-dark-border rounded-lg text-sm focus:border-teal focus:outline-none"
            ...
            <div>
              <label className="block text-sm font-semibold text-warmgray-700 dark:text-warmgray-300 mb-1">Message / Concern</label>
              <textarea
                rows={3}
                value={form.message}
                onChange={e => setForm({ ...form, message: e.target.value })}
                className="w-full px-3 py-2 border-2 border-warmgray-300 dark:border-surface-dark-border rounded-lg text-sm focus:border-teal focus:outline-none resize-none"
