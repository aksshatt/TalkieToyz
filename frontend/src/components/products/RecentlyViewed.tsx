...
const RecentlyViewed = () => {
  ...
  return (
    <section className="py-10 bg-white dark:bg-surface-dark-raised border-t border-warmgray-100 dark:border-surface-dark-border">
            ...
            return (
              ...
              >
                <Link to={`/products/${product.slug}`} className="block">
                  <div className="rounded-xl overflow-hidden mb-2 bg-warmgray-50 dark:bg-surface-dark">
                    <motion.img src={imageUrl} alt={product.name}
                      className="w-full h-32 sm:h-36 object-cover"
                      whileHover={{ scale: 1.07 }} transition={{ duration: 0.35 }}
                    />
                  </div>
                  <p className="text-xs font-semibold text-warmgray-700 dark:text-warmgray-300 line-clamp-2 leading-tight mb-1">{product.name}</p>
